...
def crf_binary_score(tag_indices, sequence_lengths, transition_params):
    ...
    if num_tags.dtype != tf.int32:
        num_tags = tf.cast(num_tags, tf.int32)
    if num_transitions.dtype != tf.int32:
        num_transitions = tf.cast(num_transitions, tf.int32)
...
def crf_forward(inputs, state, transition_params, sequence_lengths):
    ...
    range = [tf.range(tf.shape(sequence_lengths)[0]),
             tf.dtypes.cast(sequence_lengths, dtype=tf.int32)]
    idxs = tf.stack(range, axis=1)
