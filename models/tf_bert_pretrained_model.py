...
from transformers.modeling_tf_utils import shape_list, init_copy_embeddings, get_initializer, TFModelUtilsMixin
                ...
                for symbolic_weight in symbolic_weights:
                    ...
                    if saved_weight_value is not None:
                        ...
                        if K.int_shape(symbolic_weight) != saved_weight_value.shape:
                            ...
                            try:
                                all = 1
                                for n in K.int_shape(symbolic_weight):
                                    all *= n

                                print(all, K.int_shape(symbolic_weight))  # 16790016 (21862, 768)
                                print(saved_weight_value.size, saved_weight_value.shape)  # 16226304 (21128, 768)

                                array = np.reshape(saved_weight_value, saved_weight_value.size)
                                if all > saved_weight_value.size:
                                    array = np.concatenate((array, np.random.randn(all - saved_weight_value.size)),
                                                           axis=0)

                                print(array.shape)  # (16790016,)
                                array = np.reshape(array, K.int_shape(symbolic_weight))
                                print(array.shape)  # (21862, 768)
